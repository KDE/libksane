...
#include "KSaneOptInternal.moc"

// KDE includes
#include <KDebug>
#include <KLocale>

KSaneOptInternal::KSaneOptInternal(const SANE_Handle handle, const int index)
...
KSaneOptInternal::~KSaneOptInternal()
{
    if (m_data) {
        free(m_data);
        m_data = 0;
...
void KSaneOptInternal::readOption() 
{
    m_optDesc = sane_get_option_descriptor(m_handle, m_index);
}

KSaneOptInternal::Visibility KSaneOptInternal::visibility() const
{
...
bool KSaneOptInternal::needsPolling() const
{
...
const QString KSaneOptInternal::saneName() const
{
    if (m_optDesc == 0) return QString("");
    return QString(m_optDesc->name);
}

const QString KSaneOptInternal::title() const
{
    if (m_optDesc == 0) return QString("");
    return QString(i18n(m_optDesc->title));
}

const QString KSaneOptInternal::description() const
{
    if (m_optDesc == 0) return QString("");
    return QString(i18n(m_optDesc->desc));
}

bool KSaneOptInternal::writeData(void *data)
{
    SANE_Status status;
    SANE_Int res;
...
void KSaneOptInternal::readValue() {}

SANE_Word KSaneOptInternal::toSANE_Word(unsigned char *data)
{
    SANE_Word tmp;
...
void KSaneOptInternal::fromSANE_Word(unsigned char *data, SANE_Word from)
{
    // if __BYTE_ORDER is not defined we get #if 0 == 0
    #if __BYTE_ORDER == __LITTLE_ENDIAN
...
qreal KSaneOptInternal::minValue() const {return 0.0;}
qreal KSaneOptInternal::maxValue() const {return 0.0;}
qreal KSaneOptInternal::value()    const {return 0.0;}
const QString KSaneOptInternal::strValue() const {return QString();}
const QStringList KSaneOptInternal::comboStringList() const {return QStringList();}

bool KSaneOptInternal::setValue(qreal) {return false;}
bool KSaneOptInternal::setStrValue(const QString &) {return false;}

int  KSaneOptInternal::unit() const {return m_optDesc->unit;}

bool KSaneOptInternal::storeCurrentData()
{
    SANE_Status status;
    SANE_Int res;
...
bool KSaneOptInternal::restoreSavedData()
{
...
KSaneOptInternal::KSaneOptType KSaneOptInternal::optionType() const
{
    return optionType(m_optDesc);
}

KSaneOptInternal::KSaneOptType KSaneOptInternal::optionType(const SANE_Option_Descriptor *optDesc)
{
...
const KLocalizedString KSaneOptInternal::unitString() const
{
...
const QString KSaneOptInternal::unitSpinBoxDoubleString() const
{
    switch(m_optDesc->unit)
    {
